...
links.Network = function(container) {
  ...
  this.selectedPackages = [];  // array with all Selected packages Prjm@x Modification
...
links.Network.prototype._onMouseDown = function (event) {
  ...
  //Prjm@x Modification
  var overlappingPackages = this._getPackagesOverlappingWith(obj);
   this.startClickedPck = (overlappingPackages.length > 0) ? 
    overlappingPackages[overlappingPackages.length - 1] : undefined;

  if (this.startClickedPck) {      
      var selpackage = this.packages[this.startClickedPck.row];
      var packageindex = this.selectedPackages.indexOf(selpackage.uid)
      if (packageindex == -1 ) {
        this.selectedPackages.push(selpackage.uid);
      } else {
          this.selectedPackages.splice(packageindex,1);
      }     
      
      if (!selpackage.isSelected()) {
        selpackage.select();
      } else {
        selpackage.unselect();
      }
 ...
 *                               the parameter row.
 */ 
links.Network.prototype._getPackagesOverlappingWith = function (obj) {
  var overlappingPackages = [];
  
  for (var i = 0; i < this.packages.length; i++) {
    if (this.packages[i].isOverlappingWith(obj)) {
      var sel = {"row": i};
      overlappingPackages.push(sel);
    }
  }
...
links.Network.prototype._drawPackages = function(ctx) {
  ...
  for (var i = 0, iMax = packages.length; i < iMax; i++) {
    ...
    // draw the selected Packages on top
    for (var s = 0, sMax = selected.length; s < sMax; s++) {
        packages[selected[s]].draw(ctx);
    }
  }
}
...
links.Network.Package.prototype.setProperties = function(properties, constants) {
  ...
  //Prjm@x Modification
  this.uid = (properties.uid == undefined) ? this.id : properties.uid;
  if (this.network.selectedPackages.indexOf(this.uid) != -1 ) {
    this.selected = true;
  } else {
    this.selected = (properties.selected == undefined) ? false : properties.selected;
    if (this.selected) {
        this.network.selectedPackages.push(this.uid);
    }
  }
 ...
 * select this Package 
 */ 
links.Network.Package.prototype.select = function() {
  this.selected = true;
}
 ...
 */ 
links.Network.Package.prototype.unselect = function() {
  this.selected = false;
}

//Prjm@x Modification
/**
 * check if this Package is selecte
 * @return {boolean} selected   True if node is selected, else false
 */ 
links.Network.Package.prototype.isSelected = function() {
  return this.selected;
}
...
links.Network.Package.prototype._drawShape = function (ctx, shape) {
  ...
  //Prjm@x Modification
  this.radius = this.selected ? this.radius * 1.5 :this.radius;      
...
links.Network.Package.prototype._drawImage = function (ctx) {
  if (this.imageObj) {
    if (this.value) {
      ...
      //Prjm@x Modification
      var width = this.selected ? this.imageObj.width * 1.5 :this.imageObj.width;
      var height = this.selected ? this.imageObj.height * 1.5 : this.imageObj.height;
    }
    var pos = this._getPosition();
